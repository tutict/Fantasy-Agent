...
**ComfyUI 那格同理**：`_probe_comfyui` 曾自建候选端点，且跳过 `_is_local_http_endpoint` 这道本地限定——`COMFYUI_ENDPOINT` 指到远程时面板报 `ready`，而每个 run 都拒绝同一个端点（`comfyui_mcp` 的 `allow_remote_endpoint` 默认 false），顺带把设计上要显式授权的对外请求发出去了。现在它直接读 `local_tools._comfyui_target()`，与 Unreal 那格同一个套路。

**"本地端点"这道闸门只有一份实现**：`local_tools._is_local_http_endpoint` 同时查 scheme 与 host，三个调用方共用它——ComfyUI resolver、状态面板、以及 `comfyui_mcp` 的 manifest 校验。`comfyui_mcp` 原先自己写过一份**只看 host 不看 scheme** 的 `_is_local_endpoint`，于是 `file://127.0.0.1/<任意路径>` 能通过 `allow_remote_endpoint=false` 的闸门，而 `urlopen` 对 `file:` URL 是照开的——本地文件读取面（受限于请求路径都是固定后缀，实际能读到的很有限，但闸门比设计意图宽）。改法是删掉那份副本、改用共享谓词，不是再加一道检查。
...
- `--base-directory` 会要求 `custom_nodes` 目录存在，用 `--models-directory` 分别挂更省事。
- 带 torch 的解释器在 `ComfyUI/ComfyUI/.venv`，外层 `standalone-env` **没有 torch**（manifest 声称有，实际没装）。
- 上面那条命令行是 cmd / PowerShell 写法（`D:\...`），照抄可用；但**在禁用 MSYS 路径转换的 shell 里**（本仓库的工具 shell 就设着 `MSYS_NO_PATHCONV=1`）路径不会被翻译，写 `/d/Comfy-Desktop/...` 会被 python 原样收到，报 `argument --models-directory: The path ... does not exist`。**一律写 `D:/...`**（正斜杠 Windows 路径）就三边通吃，别依赖 shell 的翻译。
- **底模要自己备**：`models/checkpoints` 原本是空的，验证时下的是 `v1-5-pruned-emaonly.safetensors`（SD1.5 fp32，4.27GB）。ComfyUI 在启动时缓存模型清单，加了底模要**重启服务**才看得到。
...
路线是生成的，所以"走不通"是生成器的 bug，不是关卡设计问题。十条实测过的规则，改之前先读：
...
- **敌人只站在中间那些地砖上，且不会两个同点。** 第一块砖是玩家的出生点、最后一块砖上是出口门，站在任何一端的敌人都只是站在玩家的腿上或者门框里。曾经按 `floors[index + 1]` 往上数并用 `mini()` 钳位：stealth 轴 5 个敌人、4 块砖，于是 3 个落在出口砖上、其中两个坐标完全相同、还都在门的实心体积内。现在取 `floors.slice(1, size - 1)` 作候选，横向只占两侧车道（接触即判负，中央车道等于对着直走的玩家判负），三个计数按 (tile, lane, row) 混合进制展开，所以**任意敌人数量下都不会出现同点**。
- **spec 的文字进 GDScript 字面量前必须转义。** `_fail("{fail0}")` / `_fail("{boundary}")` 是把 spec 自己的话（模型写的）贴进生成源码，只处理引号是不够的：换行会让字面量中途断行（`_fail("Player fell off` 后面接下一行），反斜杠更糟——`\r` 是回车而 `\l` 根本不是转义，一句引用 Windows 路径的失败文案就能让 Godot 拒绝解析整个 `game_manager.gd`。统一走 `_gd_literal()`（先折叠空白，再把反斜杠加倍、双引号换成单引号）。这是**运行不起来**，比文案难看严重得多。
